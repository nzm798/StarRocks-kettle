 ...
 * 执行主转换。<br>
 * 逻辑如下：<br>
 * 1. 通过 jdbc 执行remote_rows udx（初始化数据库并在 remote_rows 上生成sql_statment）。<br>
 * 2.当传入行为第一行时，缓存每个字段的索引并创建标题格式对象。<br>
 * 3. 根据 TCP/IP 通过套接字发送一行/每次的数据。<br>
 * 4. 发出所有行后，关闭套接字连接并返回。<br>
 *
 * @author Ray Zhang
 * @since Jan-05-2010
 */
