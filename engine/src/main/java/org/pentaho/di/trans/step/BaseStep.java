...
public class BaseStep implements VariableSpace, StepInterface, LoggingObjectInterface, ExtensionDataInterface {
   ...
   * each rowset!
   *
   * putRow 用于将一行复制到备用行集 这应该优先于其他所有内容！
   * (synchronized) 如果 distributed 为 true，则仅将一行复制到输出行集一次，否则将副本发送到
   * 每行！用于实现分发和复制分发的功能
   *
   ...
   * 在 getRow 的情况下，我们通过输入行集从前面的步骤接收数据。 如果我们拆分流，我们
   * 必须将数据复制到备用拆分：行集 1 到 n。
   */
  @Override
