 ...
 * ValueMetaInterface 对象用于确定行字段的特征。它们通常是获得的
 * 来自 RowMetaInterface 对象，该对象通过调用 getInputRowMeta（） 获取。getType（） 方法返回以下之一
 * 由 ValueMetaInterface 声明的静态常量，用于指示 PDI 字段类型。每个字段类型映射到
 * 实际值对应的本机 Java 类型。
 * <p>
